...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
      ...
      const {accessToken, newRefreshToken} = await gernrateAccessAndRefreshToken(user._id)
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
     ...
     if (!coverImage.url) {
      throw new Apierror(400,"Error while uploadiong on cover image")
     }
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
     ...
     if (!avatar.url) {
      throw new Apierror(400,"Error while uploadiong on avatar")
     }
